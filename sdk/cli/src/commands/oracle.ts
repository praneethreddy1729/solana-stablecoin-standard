...
import {
  OraclePriceGuard,
  PYTH_FEED_IDS,
} from "../../../core/src/oracle";
import type { DepegAlert } from "../../../core/src/oracle";

/** Resolve a feed name alias (e.g., "USDC/USD") or pass through a raw hex ID. */
function resolveFeedId(feed: string): string {
  const upper = feed.toUpperCase();
  const known = PYTH_FEED_IDS as Record<string, string>;
  if (known[upper]) return known[upper];
  // Accept raw hex with or without 0x prefix
  if (/^(0x)?[0-9a-fA-F]{64}$/.test(feed)) return feed;
  throw new Error(
    `Unknown feed "${feed}". Use a hex feed ID or one of: ${Object.keys(PYTH_FEED_IDS).join(", ")}`
  );
}

function formatSeverity(severity: DepegAlert["severity"]): string {
  switch (severity) {
    case "WARNING":
...
oracleCommand
  ...
  .action(async (opts) => {
    try {
      ...
      guard.onAlert((alert: DepegAlert) => {
      ...
      const statusInterval = setInterval(async () => {
        try {
          ...
          if (lc) {
            ...
            console.log(
              `[${new Date().toISOString()}] $${lc.currentPrice.toFixed(6)} | ` +
...
oracleCommand
  ...
  .action((opts) => {
    if (opts.format === "json") {
      console.log(JSON.stringify(PYTH_FEED_IDS, null, 2));
    } else {
      console.log("=== Known Pyth Feed IDs ===");
      for (const [pair, id] of Object.entries(PYTH_FEED_IDS)) {
