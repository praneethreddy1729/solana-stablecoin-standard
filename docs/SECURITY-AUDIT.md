...
1. **Manual code review** -- Line-by-line review of all 17 instructions in `sss-token` and 3 instructions in `sss-transfer-hook`, focusing on account validation, access control, and arithmetic safety.
...
4. **Integration testing** -- 386 integration tests covering every instruction path, error branch, and cross-program interaction.
